        ...
        >
            <template #item="{ element: image, index }">
                ...
                >
                    ...
                    <small
                        v-if="image.deleted"
                        class="absolute left-0 bottom-0 right-0 py-1 px-2 bg-red-500 w-full text-white justify-between items-center flex"
                    >
                        Eliminata
                        <svg
                    ...
                    >
                        <svg
                            ...
                            viewBox="0 0 20 20"
                            fill="currentColor"
                            class="w-6 h-6 rounded-full bg-gray-300 text-red-700"
                        >
                            <path
...
function revertImage(image) {
    if (image.isProp) {
        deletedImages.value = deletedImages.value.filter(
            (id) => id !== image.id
        );
        image.deleted = false;
